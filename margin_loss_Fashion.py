...
# Model (on Device)
Network_model = to_device(ResNet_size(size=18, in_channels=1, output_size=output_dim, margin=margin, beta=beta), device)
...
#File_name = "Log/Margin/Fashion/" + str(output_dim) + "d/Fashion_acc-" + str(acc) + "_bs-" + str(batch_size) + "_eps-" + str(epochs) + "_lr-" + str(max_lr) + "_m-" + str(margin) + "_b-" + str(beta) + ".pth"
